/**
 * LibraSearch — Node.js Test Runner
 * run-tests.js
 *
 * Stubs browser globals then runs the validation test suite.
 */
...
group("ISBN-13 Validation", () => {
  ...
  test("Valid ISBN-13: 9780061743528",            () => assert(isValidISBN13("9780061743528")));
  test("Valid ISBN-13: 9780451524935",            () => assert(isValidISBN13("9780451524935")));
  test("Valid ISBN-13 with dashes: 978-0-06-174352-8", () => assert(isValidISBN13("978-0-06-174352-8")));
  test("Reject ISBN shorter than 13 digits",     () => assert(!isValidISBN13("978006174352")));
  test("Reject ISBN longer than 13 digits",      () => assert(!isValidISBN13("97800617435281")));
  test("All-zeros ISBN has valid checksum (sum=0, 0%10=0)", () => assert(isValidISBN13("0000000000000")));
  test("Reject alphanumeric: 978A06174352X",     () => assert(!isValidISBN13("978A06174352X")));
  test("Reject empty string",                    () => assert(!isValidISBN13("")));
  test("Reject invalid checksum: 9780061743529", () => assert(!isValidISBN13("9780061743529")));
});
...
group("Full Book Form Validation", () => {
  function isValidISBN13(isbn) {
    ...
    let s = 0;
    for (let i = 0; i < 13; i++) s += parseInt(d[i]) * (i % 2 === 0 ? 1 : 3);
    return s % 10 === 0;
  }
  ...
  test("Valid book data passes validation",      () => { const r = validateBook(goodBook); assert(r.valid, JSON.stringify(r.errors)); });
  test("Missing title fails",                    () => { const r = validateBook({ ...goodBook, title: "" }); assert(!r.valid && r.errors.title); });
  test("Single-char title fails (too short)",    () => { const r = validateBook({ ...goodBook, title: "X" }); assert(!r.valid && r.errors.title === "too short"); });
  test("Missing author fails",                   () => { const r = validateBook({ ...goodBook, author: "" }); assert(!r.valid && r.errors.author); });
  test("Invalid ISBN fails",                     () => { const r = validateBook({ ...goodBook, isbn: "1234567890123" }); assert(!r.valid && r.errors.isbn === "invalid"); });
  test("Missing genre fails",                    () => { const r = validateBook({ ...goodBook, genre: "" }); assert(!r.valid && r.errors.genre); });
  test("Future year 2099 fails",                 () => { const r = validateBook({ ...goodBook, year: 2099 }); assert(!r.valid && r.errors.year); });
  test("Missing status fails",                   () => { const r = validateBook({ ...goodBook, status: "" }); assert(!r.valid && r.errors.status); });
...
group("Search Engine", () => {
  ...
  test("Search 'orwell' in all fields returns 1 result",              () => { const r = search(books, "orwell");               assertEqual(r.length, 1); });
  test("Search 'fiction' by genre returns 2 results",                 () => { const r = search(books, "fiction", "genre");     assertEqual(r.length, 2); });
  test("Empty query returns all books",                               () => { const r = search(books, "");                    assertEqual(r.length, 4); });
  test("Search by ISBN returns correct book",                         () => { const r = search(books, "9780451524935", "isbn"); assertEqual(r.length, 1); assertEqual(r[0].title, "1984"); });
  test("Search case-insensitive: 'TOLKIEN'",                          () => { const r = search(books, "TOLKIEN", "author");    assertEqual(r.length, 1); });
  test("Search by year '1960' in all fields",                         () => { const r = search(books, "1960");                 assertEqual(r.length, 1); });
  test("Non-matching search returns 0 results",                       () => { const r = search(books, "zzznomatch");           assertEqual(r.length, 0); });
  test("Sort by title ascending: first item is '1984'",               () => { const r = sortBooks(books, "title", "asc");     assertEqual(r[0].title, "1984"); });
  test("Sort by title descending: first item is 'To Kill a Mockingbird'", () => { const r = sortBooks(books, "title", "desc"); assertEqual(r[0].title, "To Kill a Mockingbird"); });
  test("Sort by year ascending: first item is 1937",                  () => { const r = sortBooks(books, "year", "asc");      assertEqual(r[0].year, 1937); });
  test("Sort by year descending: first item is 2011",                 () => { const r = sortBooks(books, "year", "desc");     assertEqual(r[0].year, 2011); });
});
